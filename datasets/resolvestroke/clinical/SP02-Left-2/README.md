---
pretty_name: "OpenH-RF - Resolve Stroke Clinical Transcranial CEUS (SP02, Left)"
license: cc-by-4.0
task_categories:
  - other
tags:
...
# OpenH-RF - Resolve Stroke Clinical Transcranial CEUS (SP02, Left)

## Dataset Description

Contrast-enhanced ultrasound (CEUS) transcranial brain acquisition of a human
subject, acquired with SYLVER, Resolve Stroke's ultrasound device, using a 32×32
matrix probe with diverging-wave transmits at 4 kHz frame rate. The acquisition
captures a microbubble contrast-agent bolus wash-in: microbubbles are injected,
perfuse the brain vasculature through the acoustic window, and gradually wash out.
...
- Data collection method: Human subject, transcranial CEUS with microbubble
  contrast agent (bolus injection), acquired through the temporal acoustic window.
...
Single zea HDF5 file (`SP02-Left-2.hdf5`) containing all 5 clips concatenated.
...
them via `zea.File(...).custom`. 

Originally written with `zea.File.create` (zea v0.1.1). `data/raw_data` is DDC IQ (last axis
[I, Q]). The hardware time-gain compensation is baked into `raw_data`;
...
**Current OpenH-RF release:** 1 HDF5 file; 5.26 GB (5,257,625,600 bytes) stored; root `zea_version` **0.1.6**. Sizes include all HDF5 contents and use decimal units (MB = 10^6 bytes, GB = 10^9 bytes, TB = 10^12 bytes), not decoded-array memory or original-source download sizes.

- Frames: 20000 (5 clips × 4000 frames)
- Frame rate: 4000 Hz (within each clip)
- Clip duration: 1 s each
- **Stored HDF5 size:** 5.26 GB (5,257,625,600 bytes).
...
- Type: Human subject (`metadata/subject/type` = `human`)
- Subject ID: `SP02` (anonymized)
- Anatomy: Brain (transcranial), left temporal acoustic window
...
![Reference B-mode (two perpendicular sectors)](SP02-Left-2_bmode.png)
...
(`tissue_highpass`, `power_doppler`). The same display-only linear TGC (1.5 dB/cm) is
applied to the power-Doppler volume before the MIP so deep flow stays visible.

![Power-Doppler 3D MIP montage](SP02-Left-2_PD_montage.png)
...
Cartesian geometry: `mvi` in magma and the velocities in a symmetric
blue-white-red map (±0.76 m/s):

![Computed reference maps montage](SP02-Left-2_references_montage.png)
...
The dataset contains no direct personal identifiers: the subject is referenced only by
an anonymized study code, and no name, date of birth, or device/operator identifiers
are stored in the released file.
